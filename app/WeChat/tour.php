...
/*        $row = $db->query("select * from tour_project_wait_detail WHERE wx_openid=:wx_openid AND project_id=:project_id AND date(addtime)=:tempdate  ORDER BY id desc limit 0,1",
            array("wx_openid" => $fromUsername, "project_id" => $project_id, "tempdate" => date('Y-m-d')));*/
