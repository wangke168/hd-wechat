...
var _shareUrl = location.protocol + "//" + document.domain;
...
var _system={
   ...
   _guide2:function(){
      ...
      this._cover(true);
      $$("guide2").show();
      $$("guide2").style.top = (_system._scroll().y + _system._client().bh - 188) + "px";
      window.onresize=function(){_system._cover(true);$("guide2").style.top=(_system._scroll().y+_system._client().bh-188)+"px";};
     setTimeout(function(){
		 _system._cover();
		 $$("guide2").hide();
		 $$("cover").onclick = null;
		 window.onresize=null;
		 _cookie._set("GUIDE2","1",60*60*24*365*10);
	  },5*1000);
   },
   _zero:function(n){
      return n<0?0:n;
...
(function(){
   ...
   WeixinJSBridge.on('menu:share:weibo', function(argv){
	  WeixinJSBridge.invoke('shareWeibo',{
         ...
         "url": _shareUrl + dataForWeixin.path
		    ...
		    if(functionName){
		        try {
		            ...
		            if (request.responseXML) {
		                ...
		                var reqelement = reqxml.documentElement;
		                if (reqelement.innerHTML)
		                    json = eval("(" + reqelement.innerHTML + ")");
		                if (!json) {
		                    if (reqelement.text) {
		                        json = eval("(" + reqelement.text + ")");
		                    }
		                }
						if (!json) {
							if(reqelement.firstChild){
								if(reqelement.firstChild.nodeValue)
									json = eval("(" + reqelement.firstChild.nodeValue + ")");
							}
						}
		            }
		            else
